...
def test_get_revisions(call_conduit):
    ...
    with pytest.raises(ValueError):
        get_revs(repo, ids=[1], phids=[1])

    call_conduit.return_value = {}
    assert get_revs(repo, ids=[1]) is None
    call_conduit.assert_called_with(
        "differential.revision.search", dict(constraints=dict(ids=[1]))
    )
    call_conduit.reset_mock()
    assert get_revs(repo, phids=[1]) is None
    call_conduit.assert_called_with(
        "differential.revision.search", dict(constraints=dict(phids=[1]))
    )
